...
class Schema {
 public:
  ...
  bool AllEqual(std::map<std::string, int> &input) {
    bool result = true;
    for (uint32_t i = 0; result && i < columns_.size(); i++) {
      result = input.find(columns_[i]->GetName()) != input.end();
    }
    return result;
  }


  std::vector<uint32_t> &getPrimaryKeys() { return primary_keys; }
  void setPrimaryKeys(std::vector<uint32_t> &another) {
    primary_keys.resize(another.size());
    std::copy(another.begin(), another.end(), primary_keys.begin());
  }

  std::vector<uint32_t> &getUniqueKeys() { return unique_keys; }
  void setUniqueKeys(std::vector<uint32_t> &another) {
    unique_keys.resize(another.size());
    std::copy(another.begin(), another.end(), unique_keys.begin());
  }
