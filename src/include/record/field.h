 ...
 public:
  ...
  explicit Field(TypeId type, char *data, uint32_t len, bool manage_data) : type_id_(type), manage_data_(manage_data) {
    ...
    if (data == nullptr) {
      ...
      if (manage_data) {
        // manage_data指的就是是否拥有自己的字符串数组，而不是其他数组的拷贝
  ...
  inline const char *GetData() const {
    auto temp = Type::GetInstance(type_id_);
    if (temp->GetTypeId() == kTypeFloat) {
      return (dynamic_cast<TypeFloat *>(temp))->GetData(*this);
    } else if (temp->GetTypeId() == kTypeChar) {
      if (this->is_null_) {
        return nullptr;
      } else {
        return (dynamic_cast<TypeChar *>(temp))->GetData(*this);
      }
    }
    return nullptr;
  }
  inline float GetFloatData() { return value_.float_; }
  ...
  inline TypeId GetType() const noexcept { return type_id_; }
