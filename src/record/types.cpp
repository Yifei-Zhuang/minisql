...
const char *TypeFloat::GetData(const Field &val) const { return nullptr; }
float TypeFloat::GetFloatData(const Field &val) const { return val.value_.float_; }
...
CmpBool TypeFloat::CompareEquals(const Field &left, const Field &right) const {
  ...
  bool result = (fabs(left.value_.float_ - right.value_.float_) <= 1e-3);
  return GetCmpBool(result);
}
