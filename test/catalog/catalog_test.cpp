...
TEST(CatalogTest, CatalogTableTest) {
  ...
  schema->getPrimaryKeys().push_back(0);
  const_cast<Column *>(schema->GetColumn(0))->SetUnique();
  Transaction txn;
  ...
  ASSERT_FALSE(db_01->bpm_->IsPageFree(0) || db_01->bpm_->IsPageFree(1));
  delete db_01;
...
TEST(CatalogTest, CatalogIndexTest) {
  ...
  TableInfo *table_info = nullptr;
  ASSERT_EQ(DB_TABLE_NOT_EXIST, catalog_01->GetTable("table-1", table_info));
  std::vector<Column *> columns = {ALLOC_COLUMN(heap)("id", TypeId::kTypeInt, 0, false, true),
                                   ALLOC_COLUMN(heap)("name", TypeId::kTypeChar, 64, 1, true, false),
                                   ALLOC_COLUMN(heap)("account", TypeId::kTypeFloat, 2, true, false)};
  auto schema = std::make_shared<Schema>(columns);
  schema->getPrimaryKeys().push_back(0);
  ...
  // 移除索引之后，应该找不到索引
  ASSERT_EQ(DB_TABLE_NOT_EXIST, catalog_02->DropIndex("table-2", "index-2"));
  ...
  // 移除表之后，应该找不到表
  ASSERT_EQ(DB_TABLE_NOT_EXIST, catalog_02->DropTable("table-2"));
  ASSERT_EQ(DB_SUCCESS, catalog_02->DropTable("table-1"));
  ASSERT_EQ(DB_TABLE_NOT_EXIST, catalog_02->GetTable("table-1", table_info));
